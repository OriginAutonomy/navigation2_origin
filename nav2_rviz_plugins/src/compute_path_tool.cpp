...
void ComputePathTool::onPoseSet(double x, double y, double theta)
{
  if (!action_client_->wait_for_action_server(std::chrono::seconds(2))) {
    RCLCPP_ERROR(
      node_->get_logger(),
      "compute_path_to_pose action server is not available.");
    return;
  }
  ...
  goal_msg.plan_in_static = true;
  goal_msg.classify_paths = true;

  RCLCPP_INFO(
    node_->get_logger(),
    "Sending ComputePathToPose goal: (%.2f, %.2f, %.2f) in frame '%s'",
    x, y, theta, goal.header.frame_id.c_str());

  auto send_goal_options = rclcpp_action::Client<ComputePathToPose>::SendGoalOptions();
  send_goal_options.result_callback =
    [this](const rclcpp_action::ClientGoalHandle<ComputePathToPose>::WrappedResult & result) {
      switch (result.code) {
        case rclcpp_action::ResultCode::SUCCEEDED:
          RCLCPP_INFO(
            node_->get_logger(),
            "ComputePathToPose succeeded: path has %zu poses",
